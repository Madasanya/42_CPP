...
ClapTrap::ClapTrap()
	:name_("CL4P-TP"), hitPoints_(initHitPoints_), energyPoints_(initEnergyPoints_), attackDamage_(initAttackDamage_), maxHitPoints_(initHitPoints_)
{
...
ClapTrap::ClapTrap(const ClapTrap& obj)
	:name_(obj.getName()), hitPoints_(obj.hitPoints_), energyPoints_(obj.energyPoints_), attackDamage_(obj.attackDamage_), maxHitPoints_(obj.maxHitPoints_)
{
	if (COMMENTS_CANONIAL)
...
ClapTrap::ClapTrap(std::string name)
	:name_(name), hitPoints_(initHitPoints_), energyPoints_(initEnergyPoints_), attackDamage_(initAttackDamage_), maxHitPoints_(initHitPoints_)
{
	if (COMMENTS_CANONIAL)
		std::cout << COLOR_GREEN << name_ << COLOR_DEFAULT << " appears in the borderlands ..." << std::endl;
	printPoints(*this);
...
{
	if (this->hitPoints_ < 1)
		std::cout << COLOR_GREEN << name_ << COLOR_DEFAULT << " is fatally injured and therefore can't repair itself." << std::endl;
	...
	else if (this->hitPoints_ > this->maxHitPoints_)
	{
		(this->energyPoints_)--;
	...
	else
	{
		...
		std::cout << minValue(this->maxHitPoints_ - this->hitPoints_, amount) << " points." << std::endl;
		this->hitPoints_ += minValue(this->maxHitPoints_ - this->hitPoints_, amount);
	}
	printPoints(*this);
}
